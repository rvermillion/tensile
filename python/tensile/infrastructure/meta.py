...
def build_coerce(name: str = None, check_cls: type = None, field_type: FieldType = None, desc: str = '', required: bool = None) -> Optional[Coercer]:
    ...
    if check_cls is None or callable(getattr(check_cls, coerce_method, None)):
...
class Field(RootObject):
    ...
    def build_coerce(self, spec: Spec) -> Optional[Coercer]:
        return build_coerce(self.name, check_cls=self.owner.cls, field_type=self.type, desc=self.qname, required=self.required)
    ...
    def build_init(self, spec: Spec) -> Optional[Initter]:
        if self.readonly and isinstance(self.member, property):
            return None
        return name_initter(self.name, self.aliases, writer=self.write, default=self.default)
...
def register(meta: 'Meta', cls: type = None, qname: str = None, *, override: bool = False) -> None:
...
def coerce(cls: type[T], spec: Mapping[str, Any] = None, /, **kwargs) -> T:
    return Meta.for_class(cls, build=True).coerce(spec, **kwargs)
